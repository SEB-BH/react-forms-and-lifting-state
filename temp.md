...
Our component might currently look similar to this:
...
The contact display has been removed because that is now handled by the `Contacts` component.
...
Suppose the user completes the form and selects **Add Contact**.

### 1. `handleSubmit` runs inside `Form`
...
### 2. `Form` calls the function it received from `App`

```jsx
setContacts([...contacts, newContact])
```

### 3. The contacts state inside `App` changes
...
The updated contacts array is passed to both child components:
...
### 5. `Contacts` maps through the updated array

```jsx
contacts.map((contact) => (
```

The new contact appears on the page.
...
## Common error: leaving the old state in `Form`

After moving the contacts state into `App`, remove this from `Form`:

```jsx
const [contacts, setContacts] = useState([])
```

If both `App` and `Form` have their own contacts state, they will contain two separate arrays.

Updating the array in `Form` would not update the array being displayed by `Contacts`.

---

## Common error: forgetting to pass a prop

This does not give `Contacts` access to the array:
...
## Common error: calling the updater while passing it
...
React should call `setContacts` later, when the form is submitted.
